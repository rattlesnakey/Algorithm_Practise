...
void str(){
    ...
    // getline(cin,d);
    cout<<d<<endl;
    cout<<d.substr(1,3)<<endl; // 起始位置 + 个数
    cout<<d.replace(d.begin(),d.begin()+2,"k")<<endl;  // 规定指针区域
}
int main(){
	...
	set<int> iset1(begin(iarr1),end(iarr1));  //好像要multiset才可以，然后普通的数组的话可以直接begin(a), end(a)
	set<int> iset2(begin(iarr2),end(iarr2));
	vector<int> ivec(10);  
    ...
    cout<<"----------------------"<<endl;
    // heap();
    // a.insert(a.begin()+1,3);  // 记得要加上a.begin()才能去insert
    // printVec(a);
    // a.erase(a.begin()+1);
    // printVec(a);
